(function() {
    // --- Directus API Setup ---
    const DIRECTUS_URL = 'http://172.18.162.62:8055';
    const DIRECTUS_TOKEN = '-QIjtBjqqo360W4Vfj_2rkXmuLL_W7vJ';
    ...
    /**
     * Generic fetch wrapper for Directus API
     * @param {string} method - 'GET', 'POST', 'PATCH', 'DELETE'
     * @param {string} path - The API path (e.g., '/items/volunteers')
     * @param {object} [body] - The request body for POST/PATCH
     * @returns {Promise<any>} - The JSON response from the API
     */
    async function directusFetch(method, path, body = null) {
        const options = {
            method,
            headers: {
     ...
     * @returns {Promise<object>} - The API response containing the file ID.
     */
    async function uploadFile(file) {
         const formData = new FormData();
         formData.append('file', file);
         const result = await directusFetch('POST', '/files', formData);
         return result.data; // The file object, including its ID
    }
    ...
    const fieldMap = {
        id: 'id',
        fullName: 'full_name',
        assignedNumber: 'assigned_number',
        ...
        spouseContactNumber: 'spouse_contact_number',
        weddingAnniversary: 'wedding_anniversary',
        churchCongregation: 'church_congregation',
        picture: 'picture',
        children: 'children'
    };

    /**
     * Transforms a volunteer object from API format (snake_case) to app format (camelCase).
     * @param {object} apiVolunteer - The volunteer object from the Directus API.
     * @returns {object} - The volunteer object in the app's format.
     */
    function transformApiToApp(apiObject, map) {
        const appObject = {};
        for (const appKey in map) {
            const apiKey = map[appKey];
            if (apiObject[apiKey] !== undefined) {
                // Handle relational data (e.g., picture ID or children array)
                if (appKey === 'children' && Array.isArray(apiObject[apiKey])) {
                     appObject[appKey] = apiObject[apiKey].map(child => ({ name: child.name, birthDate: child.birth_date, id: child.id }));
                } else {
                    appObject[appKey] = apiObject[apiKey];
                }
            }
        }
        return appObject;
    ...
    /**
     * Transforms a volunteer object from app format (camelCase) to API format (snake_case).
     * @param {object} appVolunteer - The volunteer object from the application.
     * @returns {object} - The volunteer object ready to be sent to the API.
     */
    function transformAppToApi(appObject, map) {
        const apiObject = {};
         for (const appKey in map) {
            const apiKey = map[appKey];
            if (appObject[appKey] !== undefined && appKey !== 'id') { // Don't include ID in the body
                 if (appKey === 'children' && Array.isArray(appObject[appKey])) {
                     // Format for creating/updating relational items
                     apiObject[apiKey] = appObject[appKey].map(child => ({ name: child.name, birth_date: child.birthDate }));
                 } else {
                     // Handle null values for fields that might be empty
                     apiObject[apiKey] = appObject[appKey] === '' ? null : appObject[appKey];
                 }
            }
        }
        return apiObject;
    ...
    async function exportData() {
        try {
           const response = await directusFetch('GET', '/items/volunteers?limit=-1&fields=*,children.*' );
    ...
    async function loadVolunteers() {
        ...
        try {
            // First, fetch all volunteers
            const volunteersResponse = await directusFetch('GET', '/items/volunteers?limit=-1');
            const volunteers = volunteersResponse.data;

            // Then, fetch all children separately
            const childrenResponse = await directusFetch('GET', '/items/volunteer_children?limit=-1&fields=*');
            const allChildren = childrenResponse.data;

            // Group children by volunteer_id
            const childrenByVolunteerId = {};
            allChildren.forEach(child => {
                if (!childrenByVolunteerId[child.volunteer_id]) {
                    childrenByVolunteerId[child.volunteer_id] = [];
                }
                childrenByVolunteerId[child.volunteer_id].push({
                    id: child.id,
                    name: child.name,
            ...
            // Transform volunteers and attach their children
            allVolunteers = volunteers.map(volunteer => {
                const transformed = transformApiToApp(volunteer, fieldMap);
                // Attach children for this volunteer
                transformed.children = childrenByVolunteerId[volunteer.id] || [];
                return transformed;
            });
            ...
            showMessageBox('Error', `Failed to load volunteer data: ${error.message}`);
        }
    }
    ...
    function filterVolunteers(searchTerm) {
        ...
        let filteredVolunteers = allVolunteers.filter(v => {
            ...
            return (v.fullName && v.fullName.toLowerCase().includes(lowerCaseSearchTerm)) || 
                   (v.assignedNumber && String(v.assignedNumber).toLowerCase().includes(lowerCaseSearchTerm));
        });
        ...
        filteredVolunteers.forEach(volunteer => {
            ...
            // Ensure volunteer.picture is not empty before building the Directus asset URL
            const pictureSrc = (volunteer.picture && typeof volunteer.picture === 'string')
                ? `${DIRECTUS_URL}/assets/${volunteer.picture}?width=60&height=60&fit=cover`
                : 'nopfp.webp';
            ...
            card.innerHTML = `
                ...
                <div class="volunteer-card-header">
                    <img src="${pictureSrc}" alt="${volunteer.fullName}" 
                         class="volunteer-card-picture">
                    <div>
                        <div class="volunteer-card-name">       
                            <strong>${volunteer.assignedNumber || 'N/A'}</strong> ${volunteer.fullName}
                        </div>
                    </div>
                </div>
                <div>
    ...
    async function showVolunteerDetail(id) {
        // Find volunteer from the pre-loaded global array
        ...
        try {
            ...
            const pictureSrc = volunteer.picture ? `${DIRECTUS_URL}/assets/${volunteer.picture}` : 'https://placehold.co/150x150/cbd5e1/64748b?text=No+Pic';
            document.getElementById('detailPicture').src = pictureSrc;
            ...
            if (volunteer.children && volunteer.children.length > 0) {
                volunteer.children.forEach(child => {
                    ...
                    li.textContent = `${child.name}${childAge !== 'N/A' ? ` (${childAge} years old)` : ''}`;
                    detailChildrenList.appendChild(li);
                });
            } else {
                detailChildrenList.innerHTML = '<li>No children recorded</li>';
            }
        } catch (error) {
    ...
    window.editVolunteer = async function(id) {
        try {
            ...
            // Store picture ID in a data attribute
            const picturePreview = document.getElementById('picturePreview');
            picturePreview.dataset.existingPictureId = volunteer.picture || '';

            // Handle children - clear container first
            const childrenInfoContainer = document.getElementById('childrenInfoContainer');
            childrenInfoContainer.innerHTML = '';
            
            console.log('Loading children for edit:', volunteer.children); // Debug log
            ...
            if (volunteer.picture) {
                picturePreview.src = `${DIRECTUS_URL}/assets/${volunteer.picture}`;
    ...
    window.confirmDeleteVolunteer = function(id) {
        showMessageBox(
            ...
            async () => {
                try {
                    // Directus handles relational data deletion if schema is set up with 'CASCADE'.
                    // If not, manual deletion of related items is needed. Assuming CASCADE for simplicity.
                    // However, it's safer to delete related items manually.
                    
                    // 1. Find and delete related tasks
                    const tasksRes = await directusFetch('GET', `/items/tasks?filter[assigned_to_volunteer_id][_eq]=${id}&fields=id`);
                    if (tasksRes.data.length > 0) {
                        const taskIds = tasksRes.data.map(t => t.id);
                        await directusFetch('DELETE', '/items/tasks', taskIds);
                    }

                    // 2. Find and delete related attendance
                    const attRes = await directusFetch('GET', `/items/attendance?filter[volunteer_id][_eq]=${id}&fields=id`);
                     if (attRes.data.length > 0) {
                        const attIds = attRes.data.map(a => a.id);
                        await directusFetch('DELETE', '/items/attendance', attIds);
                    }

                    // 3. Find and delete related requests
                    const reqRes = await directusFetch('GET', `/items/requests?filter[volunteer_id][_eq]=${id}&fields=id`);
                    if (reqRes.data.length > 0) {
                        const reqIds = reqRes.data.map(r => r.id);
                        // Also need to delete request_absent_dates
                        for (const reqId of reqIds) {
                             const absentDatesRes = await directusFetch('GET', `/items/request_absent_dates?filter[request_id][_eq]=${reqId}&fields=id`);
                             if(absentDatesRes.data.length > 0) {
                                const dateIds = absentDatesRes.data.map(d => d.id);
                                await directusFetch('DELETE', '/items/request_absent_dates', dateIds);
                             }
                        }
                        await directusFetch('DELETE', '/items/requests', reqIds);
                    }
                    
                    // 4. Finally, delete the volunteer (children are deleted via CASCADE by Directus)
                    await directusFetch('DELETE', `/items/volunteers/${id}`);
    ...
    function clearVolunteerForm(hideForm = true) {
        ...
        picturePreview.style.display = 'none';
        picturePreview.src = '#';
        picturePreview.dataset.newFile = ''; // Clear stored file object
        picturePreview.dataset.existingPictureId = '';
    ...
    function handleFiles(files) {
        ...
        reader.onload = (e) => {
            picturePreview.src = e.target.result; // Show preview
            picturePreview.style.display = 'block';
        };
        reader.readAsDataURL(file);

        // Store the actual file object to be uploaded later
        picturePreview.dataset.newFile = 'true';
        window.newPictureFile = file;
    }
    ...
    async function loadAttendanceRecords() {
        ...
        try {
            // Fetch attendance and related volunteer name
            const response = await directusFetch('GET', '/items/attendance?limit=-1&sort=-date&fields=*,volunteer_id.full_name');
            ...
            records.forEach(record => {
                ...
                row.innerHTML = `
                    <td>${record.volunteer_id?.full_name || 'Unknown Volunteer'}</td>
            ...
            showMessageBox('Error', `Failed to load attendance data: ${error.message}`);
        }
    }
    ...
    async function loadTasks() {
        ...
        try {
            const response = await directusFetch('GET', '/items/tasks?limit=-1&fields=*,assigned_to_volunteer_id.full_name');
            ...
            tasks.forEach(task => {
                ...
                row.innerHTML = `
                    <td>${task.assigned_to_volunteer_id?.full_name || 'Unassigned'}</td>
                    <td>${task.project}</td>
                    <td>${task.area}</td>
            ...
            showMessageBox('Error', `Failed to load task data: ${error.message}`);
        }
    }

    window.editTask = async function(id) {
        try {
            const response = await directusFetch('GET', `/items/tasks/${id}`);
            ...
            document.getElementById('taskVolunteerSelect').value = task.assigned_to_volunteer_id;
    ...
    window.confirmDeleteTask = function(id) {
        showMessageBox('Confirm Deletion', 'Are you sure you want to delete this task?', true, async () => {
            try {
                await directusFetch('DELETE', `/items/tasks/${id}`);
    ...
    async function loadVolunteersByArea() {
        ...
        try {
            const response = await directusFetch('GET', '/items/tasks?limit=-1&fields=area,assigned_to_volunteer_id.full_name');
            const allTasks = response.data;
            const volunteersByArea = {};

            allTasks.forEach(task => {
                if (!task.assigned_to_volunteer_id) return; // Skip if volunteer is not assigned
                const area = task.area || 'Unassigned Area';
                if (!volunteersByArea[area]) {
                    volunteersByArea[area] = new Set();
                }
                volunteersByArea[area].add(task.assigned_to_volunteer_id.full_name);
            });
    ...
    async function loadRequests() {
        ...
        try {
            const response = await directusFetch('GET', '/items/requests?limit=-1&sort=-request_date&fields=*,volunteer_id.full_name,absent_dates.absent_date');
            ...
            requests.forEach(request => {
                const absentDatesDisplay = request.absent_dates && request.absent_dates.length > 0
                    ? request.absent_dates.map(d => displayDate(d.absent_date)).join(', ')
                    : 'N/A';
                const row = requestListBody.insertRow();
                row.innerHTML = `
                    <td>${request.volunteer_id?.full_name || 'Unknown'}</td>
            ...
            showMessageBox('Error', `Failed to load request data: ${error.message}`);
        }
    }
    ...
    async function loadTradeReports() {
        ...
        try {
            // Re-use the already loaded 'allVolunteers' global array
            ...
            showMessageBox('Error', `Failed to load trade report data: ${error.message}`);
        }
    }
    ...
    async function populateVolunteerDropdowns() {
        // Re-use the already loaded 'allVolunteers' global array
        const volunteers = [...allVolunteers].sort((a, b) => (a.fullName || '').localeCompare(b.fullName || ''));
    ...
    async function loadAllData() {
        await loadVolunteers(); // This is the main data fetch
        // The rest can be loaded on tab switch or use the loaded data
        populateVolunteerDropdowns();
        loadAttendanceRecords();
        loadTasks();
        loadRequests();
        loadTradeReports(); // Uses allVolunteers
        loadVolunteersByArea();
    }
    ...
    function switchTab(tabId) {
        ...
        // Refresh data for the selected tab for real-time updates
        if (tabId === 'volunteers') {
            if (document.getElementById('volunteerDetailView').classList.contains('hidden')) {
               loadVolunteers();
            }
            document.getElementById('volunteerFormContainer').classList.add('hidden');
            ...
            loadTradeReports(); // This uses the global volunteer list, so it's fast
        }
    }
    ...
    async function processImport() {
        ...
        // Use a Set to track assigned numbers being processed to prevent duplicate API calls
        const processedAssignedNumbers = new Set();
        ...
        for (const row of excelDataToImport) {
            ...
            if (!assignedNumber || !fullName || processedAssignedNumbers.has(assignedNumber)) continue;

            processedAssignedNumbers.add(assignedNumber);
            ...
            const appData = {
                ...
                children: [], // Children cannot be imported from a flat Excel file
                picture: null
            };
            
            // Check existence using the global list to avoid multiple API calls
            const existing = allVolunteers.find(v => String(v.assignedNumber) === assignedNumber);
            
            if (existing) {
                appData.id = existing.id; // Keep track of existing ID for update
                duplicateVolunteersToProcess.push({ new: appData, old: existing });
            } else {
                newVolunteersToImport.push(appData);
            }
        }
    ...
    async function finalizeImport() {
        ...
        document.querySelectorAll('#duplicateListContainer > div').forEach((div, index) => {
            ...
            if (choice === 'update') {
                const volunteerData = duplicateVolunteersToProcess[index].new;
                // Prepare for batch update
                volunteersToUpdate.push(transformAppToApi(volunteerData, fieldMap));
                updatedCount++;
            } else {
                ignoredCount++;
            }
        ...
        try {
            // Batch create new volunteers
            if (newVolunteersToImport.length > 0) {
                 const apiNewVolunteers = newVolunteersToImport.map(v => transformAppToApi(v, fieldMap));
                 await directusFetch('POST', '/items/volunteers', apiNewVolunteers);
                 newCount = newVolunteersToImport.length;
            }

            // Batch update existing volunteers
            if (volunteersToUpdate.length > 0) {
                await directusFetch('PATCH', '/items/volunteers', volunteersToUpdate);
            }
            ...
            showMessageBox('Import Error', `An error occurred during the import process: ${error.message}`);
        } finally {
    ...
    async function deleteAllData() {
        try {
            ...
            const collections = ['request_absent_dates', 'volunteer_children', 'attendance', 'tasks', 'requests', 'volunteers'];
            
            for (const collection of collections) {
                // Fetch all item IDs from the collection
                const response = await directusFetch('GET', `/items/${collection}?limit=-1&fields=id`);
                const itemIds = response.data.map(item => item.id);
                
                // If there are items, send a batch delete request
                if (itemIds.length > 0) {
                    await directusFetch('DELETE', `/items/${collection}`, itemIds);
                }
            }
            showMessageBox('Success', 'All data has been successfully deleted.');
            loadAllData();
        } catch (error) {
            showMessageBox('Error', 'An error occurred while trying to delete data: ' + error.message);
        } finally {
    ...
    // --- JSON Import/Export (unchanged logic, but uses API) ---
    ...
    document.getElementById('importDataInput').addEventListener('change', function (event) {
        ...
        reader.onload = async function (e) {
            try {
                ...
                // This function expects the exported format which is an array under the 'data' key
                if (Array.isArray(jsonData.data)) {
                    volunteers = jsonData.data;
                } else if (Array.isArray(jsonData)) {
                    volunteers = jsonData;
                } else {
                    throw new Error("Invalid format: Must be an array or an object with a 'data' key containing an array.");
                }
                
                if (volunteers.length === 0) {
                    showMessageBox('Error', "No volunteers found in the JSON file.");
                    return;
                }

                // Show progress message
                showMessageBox('Import Progress', `Starting import of ${volunteers.length} volunteers...`);
                
                // Batch process the import
                await importVolunteersInBatches(volunteers);
                
            } catch (err) {
                console.error("Import error:", err);
                showMessageBox('Error', "Failed to import data. Make sure the file is a valid JSON from a previous export. Error: " + err.message);
            }
        };
    ...
    async function importVolunteersInBatches(volunteers) {
        const BATCH_SIZE = 10; // Reduced batch size to avoid "request entity too large"
        ...
        try {
            ...
            for (let i = 0; i < volunteers.length; i += BATCH_SIZE) {
                ...
                try {
                    console.log(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1} of ${Math.ceil(volunteers.length / BATCH_SIZE)}`);
                    
                    // Separate main volunteer data from children
                    const volunteersToCreate = [];
                    const childrenToCreate = [];
                    
                    for (const volunteer of batch) {
                        // Prepare main volunteer data (without children)
                        const mainVolunteerData = { ...volunteer };
                        const volunteerChildren = mainVolunteerData.children || [];
                        delete mainVolunteerData.children; // Remove children from main data
                        delete mainVolunteerData.id; // Remove ID to let Directus generate new ones
                        
                        // Remove picture reference to avoid foreign key errors
                        // Pictures from exports won't exist in the new instance
                        delete mainVolunteerData.picture;
                        
                        volunteersToCreate.push(mainVolunteerData);
                        
                        // We'll handle children after volunteers are created
                        if (volunteerChildren.length > 0) {
                            childrenToCreate.push({
                                originalIndex: volunteersToCreate.length - 1,
                    ...
                    const createResponse = await directusFetch('POST', '/items/volunteers', volunteersToCreate);
                    const createdVolunteers = createResponse.data;
                    
                    // Now create children for the volunteers that have them
                    if (childrenToCreate.length > 0) {
                        const allChildrenData = [];
                        
                        childrenToCreate.forEach(item => {
                            const volunteerId = createdVolunteers[item.originalIndex].id;
                            item.children.forEach(child => {
                                allChildrenData.push({
                                    volunteer_id: volunteerId,
                                    name: child.name,
                                    birth_date: child.birth_date || child.birthDate // Handle both formats
                                });
                            });
                        });
                        
                        if (allChildrenData.length > 0) {
                            await directusFetch('POST', '/items/volunteer_children', allChildrenData);
                        }
                    }
                    
                    totalProcessed += batch.length;
                    console.log(`Successfully processed batch. Total processed: ${totalProcessed}/${volunteers.length}`);
            ...
            // Show final results
            let message = `Import completed. ${totalProcessed} volunteers processed successfully.`;
            if (totalErrors > 0) {
                message += ` ${totalErrors} volunteers failed to import.`;
                console.error('Import errors:', errors);
            }
            
            // Add note about pictures not being imported
            if (totalProcessed > 0) {
                message += ' Note: Volunteer pictures were not imported and will need to be uploaded manually.';
            }
            
            showMessageBox('Import Complete', message);
            loadAllData(); // Refresh all data
            
        } catch (error) {
            console.error('Critical import error:', error);
            showMessageBox('Import Error', 'A critical error occurred during import: ' + error.message);
        }
    }
    
    // --- Application Initialization ---
    document.addEventListener('DOMContentLoaded', async () => {
        // No DB to open, just attach listeners
        ...
        viewHistoryBtn.addEventListener('click', async () => {
             const volunteer = allVolunteers.find(v => v.id === currentDetailVolunteerId);
        ...
        volunteerForm.addEventListener('submit', async (e) => {
            ...
            // 1. Gather children data from the form separately.
            const childrenFromForm = Array.from(childrenInfoContainer.querySelectorAll('.child-entry')).map(row => ({
                name: row.querySelector('.child-name').value.trim(),
                birthDate: row.querySelector('.child-birth-date').value
            })).filter(child => child.name); // Only include children with a name.

            // 2. Gather main volunteer data, WITHOUT the children array.
            const appVolunteer = {
                ...
                // NOTE: The 'children' key is intentionally omitted here.
            };

            try {
                 // Handle picture upload/update first
                 if (picturePreview.dataset.newFile === 'true' && window.newPictureFile) {
                    const uploadedFile = await uploadFile(window.newPictureFile);
                    appVolunteer.picture = uploadedFile.id; // Link the new file ID
                 } else if (volunteerId) {
                    appVolunteer.picture = picturePreview.dataset.existingPictureId || null;
                 } else {
                    appVolunteer.picture = null;
                 }

                 // Convert main app data to API format (snake_case)
                 const apiVolunteerPayload = transformAppToApi(appVolunteer, fieldMap);

                let savedVolunteerId;

                if (volunteerId) { // --- This is an UPDATE ---
                    // 3a. PATCH the main volunteer data first.
                    await directusFetch('PATCH', `/items/volunteers/${volunteerId}`, apiVolunteerPayload);
                    savedVolunteerId = parseInt(volunteerId);

                    // 4a. Explicitly DELETE the old children records.
                    const existingVolunteer = allVolunteers.find(v => v.id === savedVolunteerId);
                    if (existingVolunteer && existingVolunteer.children && existingVolunteer.children.length > 0) {
                        const childIdsToDelete = existingVolunteer.children.map(c => c.id);
                        if (childIdsToDelete.length > 0) {
                            await directusFetch('DELETE', '/items/volunteer_children', childIdsToDelete);
                        }
                    }
                } else { // --- This is a CREATE ---
                    // 3b. POST the main volunteer data to get the new ID.
                    const createResponse = await directusFetch('POST', '/items/volunteers', apiVolunteerPayload);
                    savedVolunteerId = createResponse.data.id;
                }

                // 5. For both UPDATE and CREATE, now create the new children records directly.
                if (childrenFromForm.length > 0) {
                    const childrenPayload = childrenFromForm.map(child => ({
                        volunteer_id: savedVolunteerId, // Link to the parent
                        name: child.name,
                        birth_date: child.birthDate || null
                    }));
                    // Batch-create all children in a single API call.
                    await directusFetch('POST', '/items/volunteer_children', childrenPayload);
                }

                showMessageBox('Success', `Volunteer ${volunteerId ? 'updated' : 'added'} successfully!`);
                ...
                window.newPictureFile = null;
                picturePreview.dataset.newFile = 'false';
            }
        });
        ...
        clearPictureBtn.addEventListener('click', () => {
            ...
            picturePreview.style.display = 'none';
            picturePreview.src = '#';
            picturePreview.dataset.newFile = 'true'; // Mark as changed to clear it
            window.newPictureFile = null; // Ensure file object is cleared
        ...
        recordAbsencesBtn.addEventListener('click', async () => {
            ...
            for (const entry of entries) {
                ...
                // Find volunteer from global list
                const volunteer = allVolunteers.find(v => String(v.assignedNumber) === assignedNumber);
            ...
            if (recordsToCreate.length > 0) {
                try {
                   await directusFetch('POST', '/items/attendance', recordsToCreate);
                   showMessageBox('Success', `${successCount} absentee(s) recorded.`);
                   absenteeInputContainer.innerHTML = '';
                   addAbsenteeInputRow();
                   loadAttendanceRecords();
                } catch(error) {
                   showMessageBox('Error', `Failed to record absences: ${error.message}`);
                }
            } else if (entries.length > 0) {
                showMessageBox('Warning', 'No valid volunteer assigned numbers found for the entries provided.');
            }
        });
        ...
        taskForm.addEventListener('submit', async (e) => {
            ...
            const task = {
                assigned_to_volunteer_id: parseInt(volunteerSelect.value),
            ...
            try {
                if (taskId) {
                    await directusFetch('PATCH', `/items/tasks/${taskId}`, task);
                    showMessageBox('Success', 'Task updated successfully!');
                } else {
                    await directusFetch('POST', '/items/tasks', task);
        ...
        requestForm.addEventListener('submit', async (e) => {
            ...
            const newRequest = {
                ...
                status: 'Pending',
                // Format for deep insert of relational dates
                absent_dates: absentDates.map(date => ({ absent_date: date }))
            };

            try {
                await directusFetch('POST', '/items/requests', newRequest);
                showMessageBox('Success', 'Request submitted!');
                requestForm.reset();
                document.getElementById('absenceDatesSection').classList.add('hidden');
                document.getElementById('absentDatesContainer').innerHTML = '';
                loadRequests();
            } catch (error) {
                showMessageBox('Error', `Failed to submit request: ${error.message}`);
            }
        });
        requestTypeSelect.addEventListener('change', (e) => {
            const isLeave = e.target.value === 'Leave Request';
            document.getElementById('absenceDatesSection').classList.toggle('hidden', !isLeave);
             if (isLeave && document.getElementById('absentDatesContainer').children.length === 0) {
                addAbsentDateInput();
            }
        });
        function addAbsentDateInput() {
            const container = document.getElementById('absentDatesContainer');
            const newRow = document.createElement('div');
            newRow.className = 'flex items-center space-x-2 absent-date-entry';
            newRow.innerHTML = `
                 <input type="date" class="absent-date-input flex-grow p-2 border rounded-md">
                 <button type="button" class="remove-absent-date-row text-red-500 hover:text-red-700 font-bold">&times;</button>
            `;
        ...
        clearRequestFormBtn.addEventListener('click', () => {
            requestForm.reset();
            document.getElementById('absenceDatesSection').classList.add('hidden');
            document.getElementById('absentDatesContainer').innerHTML = '';
        });
        requestListBody.addEventListener('change', async (e) => {
            if (e.target.classList.contains('request-status-select')) {
                ...
                try {
                    await directusFetch('PATCH', `/items/requests/${requestId}`, { status: newStatus });
                     showMessageBox('Success', 'Request status updated.');
                } catch (error) {
                    showMessageBox('Error', 'Failed to update status.');
         ...
         requestListBody.addEventListener('click', async (e) => {
            if (e.target.classList.contains('request-delete-btn')) {
                 ...
                 showMessageBox('Confirm Deletion', 'Are you sure you want to delete this request?', true, async () => {
                    try {
                         // First delete related absent dates
                         const absentDatesRes = await directusFetch('GET', `/items/request_absent_dates?filter[request_id][_eq]=${requestId}&fields=id`);
                         if(absentDatesRes.data.length > 0) {
                            const dateIds = absentDatesRes.data.map(d => d.id);
                            await directusFetch('DELETE', '/items/request_absent_dates', dateIds);
                         }
                         // Then delete the request itself
                         await directusFetch('DELETE', `/items/requests/${requestId}`);
                         showMessageBox('Success', 'Request deleted.');
                         loadRequests();
                    } catch (error) {
                        showMessageBox('Error', `Failed to delete request: ${error.message}`);
                    }
                 });
            }
        });
        ...
        try {
            ...
            switchTab('volunteers');
             ...
             showMessageBox('Connection Error', 'Could not connect to the backend server. Please check the connection and refresh the page. ' + error.message);
        }
    });
})();